...
const emit = defineEmits<{ "new-booking-added": () => void }>();

const isModalOpen = defineModel<boolean>();

//todo alert about unsaved changes (composable???)
...
const schema = z.object({
  ...
  customerAge: z.number().positive({ message: "Please provide a valid customer age" }),
})
          ...
          <UCard @submit.prevent="() => onSubmit(bookingForm)">
            ...
            <div v-else-if="item.key === 'customerInfo'" class="space-y-3">
              ...
              <UFormGroup label="Customer name" name="customerName" required>
                <UInput v-model="bookingForm.customerName" type="text" required />
              </UFormGroup>
                
              <UFormGroup label="Customer email" name="customerEmail" required>
                <UInput v-model="bookingForm.customerEmail" type="email" required />
              </UFormGroup>
                
              <UFormGroup label="Customer phone number" name="customerPhone" required>
                <UInput v-model="bookingForm.customerPhone" type="number" required />
              </UFormGroup>
                
              <UFormGroup label="Customer age" name="customerAge" required>
                <UInput v-model="bookingForm.customerAge" type="number" required />
              </UFormGroup>
                
              <UFormGroup label="Customer gender" name="customerGender" required>
                <USelect v-model="bookingForm.customerGender" :options="genders" option-attribute="name" required/>
              </UFormGroup>
            ...
            <div v-else-if="item.key === 'paymentType'" class="space-y-3">
              ...
              >
                <UFormGroup label="Payment type" name="paymentType" required>
                  <USelect v-model="bookingForm.paymentType" :options="paymentTypes" option-attribute="name" required/>
                </UFormGroup>
